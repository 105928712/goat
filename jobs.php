<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="author" content="G.O.A.T Team - Joey Manani & Nima Adel">
    <meta name="description" content="View all of our current job vacancies. We'd love you to be part of our team. We're GOAT Solutions. Tomorrow's Innovation, Today.">
    <meta name="keywords" content="GOAT, melbourne, IT, software, development">
    <title>Job Vacancies | GOAT Solutions</title>
    <link rel="stylesheet" href="styles/styles.css">
</head>
<body>
    <!-- Navigation Bar -->
    <?php include 'includes/header.inc'; ?>

    <main>
        <!-- Create a page title panel -->
        <section class="panel">
            <h1 class="jobs-page-title">Job Openings</h1>
            <h2>There are currently 5 vacancies</h2>
        </section>

        <!-- Create a grid layout for the job listings -->
        <div class="jobs-grid">
            <!-- Job 1: Software Developer -->
            <section class="panel">
                <h2 class="job-title">SDEV1</h2>
                <h3 class="job-title-extended">Software Developer</h3>
                <p class="job-description">
                    This position requires the designing, building, testing, and maintenance of computer applications and programs. The role also involves translating user and client needs and requirements into functional code, while also identifying and resolving issues. Software Developers also ensure software performance, security, and reliability.
                </p>
                <p class="salary-range"><strong>Salary Range:</strong> Up to $90,000 for entry level positions</p>
                <p class="report-to"><strong>Applicants Report To:</strong> Senior Software Developer (Level 3 Position)</p>
                <h3>Key Responsibilities:</h3>
                <ul class="job-responsibilities">
                    <li>To develop efficient and effective software that meets user and clients' needs</li>
                    <li>Ensuring that projects are secure with the necessary protections</li>
                    <li>Maintaining and updating existing software applications</li>
                    <li>Providing technical support to users and stakeholders</li>
                    <li>Testing software applications to identify and resolve bugs</li>
                    <li>Ensures software meets quality standards and performance requirements</li>
                </ul>
                <h3><u>Required</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="required-skills">
                    <li>Understanding the conventions within software and specific programming languages, including sorting and searching algorithms, binary, HTTP, interoperability, abstractions and object oriented programming.</li>
                    <li>Experience with at least TWO of the following languages: JavaScript, HTML/CSS, Python, SQL, Java, C#</li>
                    <li>Knowledge of software development methodologies including including Agile</li>
                    <li>Familiarity with databases and data structures</li>
                    <li>Problem Solving: Understanding how to identify, diagnose, and break down a problem where innovative solutions can be created</li>
                    <li>Teamwork: The ability to work well in a team for bigger projects</li>
                    <li>Communication: The ability to effectively communicate with colleagues and management</li>
                </ul>
                <h3><u>Preferred</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="preferred-skills">
                    <li>Experience working in a Software Development related position:
                        <ol class="no-number">
                            <li>3 or more years</li>
                            <li>2 or more years</li>
                            <li>1 or more years</li>
                        </ol>
                    </li>
                    <li>Knowledge of all the following coding languages and/or more: JavaScript, HTML/CSS, Python, SQL, Java, and C#</li>
                </ul>
            </section>
        
            <!-- Job 2: Network Administrator -->
            <section class="panel">
                <h2 class="job-title">NETA1</h2>
                <h3 class="job-title-extended">Network Administrator</h3>
                <p class="job-description">
                    This position involves installing, maintaining, and troubleshooting computer networks and systems, as well as ensuring smooth operation and security. These networks and systems are used to house applications, software, and expansive amounts of data. This role also involves the monitoring of changes in a company's network, and providing troubleshooting and technical support where problems arise.
                </p>
                <p class="salary-range"><strong>Salary Range:</strong> Up to $92,000 for entry level positions</p>
                <p class="report-to"><strong>Applicants Report To:</strong> Senior Network Administrator (Level 3 Position)</p>
                <h3>Key Responsibilities:</h3>
                <ul class="job-responsibilities">
                    <li>The installation, configuration, and maintenance of network hardware and software, including routers, switches, firewalls, and servers</li>
                    <li>The maintenance of accurate documentation of network configurations, procedures, and troubleshooting steps</li>
                    <li>Implementation and maintenance of security measures to protect the network and systems from unauthorized access and cyber threats</li>
                    <li>To manage our company's inventory by tracking and managing network hardware and software assets</li>
                    <li>The monitoring of network performance, including identifying possible issues within the network and systems, and the implementation of solutions to improve efficiency</li>
                </ul>
                <h3><u>Required</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="required-skills">
                    <li>Knowledge of operating systems, primarily Windows</li>
                    <li>Familiarity and understanding of network hardware, including routers, switches, firewalls, and servers</li>
                    <li>Proficiency in network monitoring and management tools, primarily SolarWinds</li>
                    <li>Problem Solving: Understanding how to identify, diagnose, and break down a problem where innovative solutions can be created</li>
                    <li>Teamwork: The ability to work well in a team for bigger projects</li>
                    <li>Communication: The ability to effectively communicate with colleagues and management</li>
                </ul>
                <h3><u>Preferred</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="preferred-skills">
                    <li>Experience working in a Network Administrator related position:
                        <ol class="no-number">
                            <li>3 or more years</li>
                            <li>2 or more years</li>
                            <li>1 or more years</li>
                        </ol>
                    </li>
                    <li>Knowledge of operating systems other than Windows, such as macOS and Linux</li>
                    <li>Advanced understanding of network hardware, including routers, switches, firewalls, and servers</li>
                    <li>Advanced understanding of our company's network management tool, SolarWinds</li>
                </ul>
            </section>
        
            <!-- Job 3: Data Analyst -->
            <section class="panel">
                <h2 class="job-title">DAAN1</h2>
                <h3 class="job-title-extended">Data Analyst</h3>
                <p class="job-description">
                    This position involves collecting, analysing, and interpreting data to identify trends, solve problems, and inform company decisions. This role takes our company’s raw data, and works to find ways to make it easier for others to understand, as well as finding trends and making predictions of what might happen in the company’s future. Data Analysts help businesses turn extensive, unorganised collections of information into useful tools to help with company forecasting.
                </p>
                <p class="salary-range"><strong>Salary Range:</strong> Up to $85,000 for entry level positions</p>
                <p class="report-to"><strong>Applicants Report To:</strong> Senior Data Analyst (Level 3 Position)</p>
                <h3>Key Responsibilities:</h3>
                <ul class="job-responsibilities">
                    <li>The creation and maintenance of information architecture for data sets, as well as ensuring easy accessibility of the information provided</li>
                    <li>Making recommendations on how the organisation can best use the data they have collected to their advantage or making suggestions as to what new data they should collect</li>
                    <li>Looking at how other businesses in our industry are using their data analysis tools</li>
                    <li>Tying together previously unrelated sets of information so that they are easier to analyse</li>
                </ul>
                <h3><u>Required</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="required-skills">
                    <li>Experience with virtualisation technologies such as charts, graphs and plots to represent data</li>
                    <li>Familiarity with databases and data structures</li>
                    <li>Problem Solving: Understanding how to identify, diagnose, and break down a problem where innovative solutions can be created</li>
                    <li>Teamwork: The ability to work well in a team for bigger projects</li>
                    <li>Communication: The ability to effectively communicate with colleagues and management</li>
                </ul>
                <h3><u>Preferred</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="preferred-skills">
                    <li>Experience working in a Data Analysis related position:
                        <ol class="no-number">
                            <li>3 or more years</li>
                            <li>2 or more years</li>
                            <li>1 or more years</li>
                        </ol>
                    </li>
                    <li>Adept knowledge of Microsoft Excel</li>
                </ul>
            </section>

            <!-- Job 4: Cybersec Specialist -->
            <section class="panel">
                <h2 class="job-title">CSSP1</h2>
                <h3 class="job-title-extended">Cybersecurity Specialist</h3>
                <p class="job-description">
                    This position involves protecting a company's computer system and network from cyber threats by identifying vulnerabilities, implementing security measures, and responding to security incidents. The role also requires the creation, installation, continuous monitoring and maintenance of company security systems.
                </p>
                <p class="salary-range"><strong>Salary Range:</strong> Up to $105,000 for entry level positions</p>
                <p class="report-to"><strong>Applicants Report To:</strong> Senior Cybersecurity Specialist (Level 3 Position)</p>
                <h3>Key Responsibilities:</h3>
                <ul class="job-responsibilities">
                    <li>Identifying potential vulnerabilities in software, hardware, networks, and data centers, then assessing the risks associated with those vulnerabilities</li>
                    <li>The designing, implementation, and maintenance of security systems, including firewalls, intrusion detection systems, and access control measures</li>
                    <li>Respond to security incidents, such as malware infections, data breaches, and denial-of-service attacks</li>
                </ul>
                <h3><u>Required</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="required-skills">
                    <li>Knowledge and understanding of networking, operating systems, security protocols, and security tools.</li>
                    <li>The ability to analyse data, identify patterns, and make reasonable and logical judgments about security threats</li>
                    <li>Problem Solving: Understanding how to identify, diagnose, and break down a problem where innovative solutions can be created</li>
                    <li>Teamwork: The ability to work well in a team for bigger projects</li>
                    <li>Communication: The ability to effectively communicate with colleagues and management</li>
                </ul>
                <h3><u>Preferred</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="preferred-skills">
                    <li>Experience working in a Cybersecurity Specialist related position:
                        <ol class="no-number">
                            <li>3 or more years</li>
                            <li>2 or more years</li>
                            <li>1 or more years</li>
                        </ol>
                    </li>
                    <li>Knowledge of operating systems other than Windows, such as macOS and Linux</li>
                    <li>Advanced understanding of network and security software tools</li>
                </ul>
            </section>

            <!-- Job 4: Cloud Engineer -->
            <section class="panel">
                <h2 class="job-title">CLEN1</h2>
                <h3 class="job-title-extended">Cloud Engineer</h3>
                <p class="job-description">
                    This position involves designing, implementing, and managing cloud-based infrastructure and services, ensuring efficient data storage, security, and scalability. This role also involves troubleshooting and optimizing cloud systems and applications.
                </p>
                <p class="salary-range"><strong>Salary Range:</strong> Up to $105,000 for entry level positions</p>
                <p class="report-to"><strong>Applicants Report To:</strong> Senior Cloud Engineer (Level 3 Position)</p>
                <h3>Key Responsibilities:</h3>
                <ul class="job-responsibilities">
                    <li>Assessing the infrastructure of a company's technological systems and making necessary migrations to the cloud</li>
                    <li>To oversee the working standards of cloud-based systems and improving them when necessary</li>
                    <li>Establish and correct the configuring of various processes, including computer, network, and security systems</li>
                    <li>Ensuring all the necessary security issues are managed, including the need to keep company data protected in the cloud</li>
                </ul>
                <h3><u>Required</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="required-skills">
                    <li>Proficiency in cloud platforms, primarily Microsoft Azure</li>
                    <li>Understanding of networking concepts, conventions and technologies.</li>
                    <li>Knowledge of server administration on operating systems, specifically Windows</li>
                    <li>Familiarity with database technologies and how to manage them</li>
                    <li>Problem Solving: Understanding how to identify, diagnose, and break down a problem where innovative solutions can be created</li>
                    <li>Teamwork: The ability to work well in a team for bigger projects</li>
                    <li>Communication: The ability to effectively communicate with colleagues and management</li>
                </ul>
                <h3><u>Preferred</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="preferred-skills">
                    <li>Experience working in a Cloud Engineering related position:
                        <ol class="no-number">
                            <li>3 or more years</li>
                            <li>2 or more years</li>
                            <li>1 or more years</li>
                        </ol>
                    </li>
                    <li>Advanced understanding of networking concepts, conventions and technologies</li>
                </ul>
            </section>

            <!-- Job 6: IT Support Technician -->
            <section class="panel">
                <h2 class="job-title">ITST1</h2>
                <h3 class="job-title-extended">IT Support Technician</h3>
                <p class="job-description">
                    This position involves technical assistance to end-users and other IT professionals within our company, ensuring the smooth functionality of computer systems, networks, and other related technologies. This role also handles troubleshooting, software and hardware installations, and provides ongoing end-user training through remote support, phone, and email.
                </p>
                <p class="salary-range"><strong>Salary Range:</strong> Up to $70,000 for entry level positions</p>
                <p class="report-to"><strong>Applicants Report To:</strong> Senior IT Support Technician (Level 3 Position)</p>
                <h3>Key Responsibilities:</h3>
                <ul class="job-responsibilities">
                    <li>Identifying and resolving technical issues with computer hardware, software, and network connectivity through troubleshooting</li>
                    <li>Establishing new software and hardware, including peripherals like monitors and printers</li>
                    <li>Assisting and supporting users of our software with technical problems, answering questions, and providing guidance on using software and hardware</li>
                    <li>Documenting and maintaining records of issues, solutions, and actions taken</li>
                    <li>Providing training and support to users on software and hardware</li>
                </ul>
                <h3><u>Required</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="required-skills">
                    <li>Strong understanding of computer hardware, software, and network concepts</li>
                    <li>Providing friendly and helpful customer service support to users of our software</li>
                    <li>Understanding how to troubleshoot and resolve issues remotely through tools like remote desktop software</li>
                    <li>The ability to adapt to and learn new technologies and software, as well as adapting to differences in users who need assistance</li>
                    <li>Problem Solving: Understanding how to identify, diagnose, and break down a problem where innovative solutions can be created</li>
                    <li>Teamwork: The ability to work well in a team for bigger projects</li>
                    <li>Communication: The ability to effectively communicate with colleagues and management</li>
                </ul>
                <h3><u>Preferred</u> Qualifications, Skills, Knowledge, and Attributes:</h3>
                <ul class="preferred-skills">
                    <li>Experience working in an IT Support Technician related position:
                        <ol class="no-number">
                            <li>3 or more years</li>
                            <li>2 or more years</li>
                            <li>1 or more years</li>
                        </ol>
                    </li>
                    <li>Advanced knowledge of troubleshooting processes and tools used in those processes</li>
                </ul>
            </section>
        </div>
        <aside class="panel">
            <h2>Experience Award-Winning Training</h2>
            <p>At GOAT, we prioritise your training and wellbeing. We have an award-winning training process which 
                will guarantee your satisfaction. Whether you're just starting out, or completely dynamic, 
                we will get you up to scratch with how we want to work with you. 
                Experience high-pay as well as a supportive and friendly workplace environment. 
                We're GOAT. Tomorrow's Innovation, Today</p>
            <a href="apply.html">Apply Now</a>
        </aside>
    </main>

    <!-- Footer -->
    <?php include 'includes/footer.inc'; ?>
</body>
</html>
